...
{
	int packets_in_buf = 4096;
	const char *payload = (const char[PACKET_SIZE]){0};
	int payload_sz = PACKET_SIZE;
	memcpy(payload, "test", 5);

	if (argc == 1) {
		error("Usage: %s [target ip:port] [target ...]", argv[0]);
	}
	...
	for (t = 0; t < thread_num; t++) {
		...
		state->src_port = 11404+t;

		char sender_name[32];
		snprintf(sender_name, 32, "udpsender-%d", t);
		thread_spawn("udpsender", thread_loop, NULL, state, t % MAX_CORES);
	}
